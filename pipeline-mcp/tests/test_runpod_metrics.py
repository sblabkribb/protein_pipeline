...
class TestRunPodMetricsStore(unittest.TestCase):
    ...
    def test_prune_old_data_keeps_recent_rows_only(self) -> None:
        ...
        store.record_usage_samples(
            [
                {"endpoint_id": "ep-1", "t": (now - timedelta(days=3)).isoformat().replace("+00:00", "Z"), "workers": 1},
                {"endpoint_id": "ep-1", "t": now.isoformat().replace("+00:00", "Z"), "workers": 2},
            ]
        )
        store.record_billing_records(
            [
                {"endpoint_id": "ep-1", "t": (now - timedelta(days=3)).isoformat().replace("+00:00", "Z"), "cost": 1.0, "records": 1},
                {"endpoint_id": "ep-1", "t": now.isoformat().replace("+00:00", "Z"), "cost": 2.0, "records": 1},
            ]
        )
        store.prune_old_data(now=now)
        usage, _resolution = store.read_usage_history(days=7, resolution="raw")
        billing, _resolution = store.read_billing_history(days=7, resolution="day")
    ...
    def test_read_usage_history_supports_daily_rollup(self) -> None:
        ...
        store.record_usage_samples(
            [
                {"endpoint_id": "ep-1", "t": "2026-03-09T01:00:00Z", "workers": 1, "queued": 0, "running": 1},
                {"endpoint_id": "ep-1", "t": "2026-03-09T10:00:00Z", "workers": 3, "queued": 2, "running": 2},
                {"endpoint_id": "ep-1", "t": "2026-03-10T02:00:00Z", "workers": 2, "queued": 1, "running": 1},
            ]
        )
        history, resolution = store.read_usage_history(days=7, resolution="day")
    ...
    def test_read_billing_history_supports_monthly_rollup(self) -> None:
        ...
        store.record_billing_records(
            [
                {"endpoint_id": "ep-1", "t": "2026-01-05T00:00:00Z", "cost": 1.5, "records": 1},
                {"endpoint_id": "ep-1", "t": "2026-01-21T00:00:00Z", "cost": 2.0, "records": 1},
                {"endpoint_id": "ep-1", "t": "2026-02-03T00:00:00Z", "cost": 4.0, "records": 1},
            ]
        )
        ...
        self.assertEqual(
            history["ep-1"],
            [
                {"t": "2026-01-01T00:00:00Z", "cost": 3.5, "records": 2, "mode": "rest"},
                {"t": "2026-02-01T00:00:00Z", "cost": 4.0, "records": 1, "mode": "rest"},
            ],
        )

    def test_normalize_health_sample_does_not_promote_warm_workers_to_running(self) -> None:
