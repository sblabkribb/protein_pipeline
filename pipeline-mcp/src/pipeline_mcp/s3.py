...
class NCPStorage:
    def __init__(self):
        ...
        if not self.access_key or not self.secret_key:
            self.client = None
            print("Warning: NCP S3 credentials not found. Remote sync disabled.")
        else:
    ...
    def upload_file(self, local_path, remote_path=None):
        ...
        try:
            self.client.upload_file(str(local_path), self.bucket_name, str(remote_path))
            return True
    ...
    def download_model(self, model_name, local_dest="pipeline-mcp/models"):
        ...
        try:
            self.client.download_file(self.bucket_name, f"models/{model_name}", str(dest_path))
            return True
        except Exception:
            return False
