...
def main() -> int:
    ...
    ss_all = primary["sum_of_squares"]["structural_pass"]
    report["result_roles"] = {
        "primary": {
            "cohort": "RFD3 전용 균형 (전향 홀드아웃, 미지 타겟)",
            "values": {"target": ss_all["target_level"],
                       "backbone_within_target": ss_all["backbone_within_target"],
                       "sequence_within_backbone": ss_all["sequence_within_backbone"]},
            "attributed_to_target_and_backbone": ss_all["attributed_to_target_and_backbone"],
            "why_primary": "타겟 -> 백본 -> 서열이 완전 균형이고 source 가 상수이며, "
                           "yield 를 보고 고른 것이 없다.",
        },
        "supplementary_consistency": {
            "cohort": "온도 패널 (20 타겟). 패널 2 가 중간 yield 백본을 선별해 구성됐다.",
            "values": {"target": 0.495, "backbone_within_target": 0.147,
                       "sequence_within_backbone": 0.358},
            "attributed_to_target_and_backbone": 0.642,
            "role": "consistency check - 주 결과를 대체하거나 보강하는 독립 증거가 "
                    "아니라, 선별이 결론을 만들지 않았는지 보는 대조다.",
            "reading": "선별된 코호트 64.2% 와 균형 코호트 "
                       f"{ss_all['attributed_to_target_and_backbone']:.1%} 가 "
                       "가깝다. 선별을 걷어내도 귀속이 거의 그대로라는 뜻이고, "
                       "따라서 주 결과가 패널 2 의 구성 방식에서 나온 것이 아니다.",
            "do_not_write": "두 값을 독립 반복으로 제시하면 안 된다. 코호트가 다르고 "
                            "한쪽은 선별돼 있다.",
        },
    }
    report["limits"] = [
        "타겟 12 개다. 분산성분 추정의 불확실성이 그만큼 크다 - 특히 타겟 수준 "
        "성분은 클러스터 12 개로 추정한 것이라 점추정을 좁게 읽으면 안 된다. "
        "이 한계는 코호트를 균형으로 바꿔도 없어지지 않는다.",
