...
타겟+백본 귀속: **63.6%** (현행) / 64.2% (대체됨). 선별을 걷어내도 값이 거의
...
**예산은 상한이지 지출이 아니다.** 적응 정책은 movability 가 바닥인 백본을
버리고 그만큼 예산을 남긴다 (평균 0.76 개 포기). 예산 100 에서 적응은 89.95
호출만 쓰고도 정적 배분(100 호출 전부 사용)보다 4.6 개를 더 찾았다 - 계산을 10%
덜 쓰고 결과는 더 낸 것이다. 예산 120 에서 차이가 −0.24 로 뒤집히는 것도 같은
이유다: 106.4 호출만 쓰고 0.24 개를 덜 찾았다. 포화 지점에서는 "찾은 수" 지표가
정책을 평가하지 못한다 - 안 쓰기로 한 결정을 손실로 세기 때문이다.
