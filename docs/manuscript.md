...
*Table 7. Structural-context ablation summary. Values are target-level means over evaluable target-arm pairs and one replicate per target-arm condition. BioEmu-containing rows include four RMSD-gate-passing targets, whereas single and RFD3 rows include all eight corrected-chain targets; paired contrasts are therefore interpreted within common target subsets.*
